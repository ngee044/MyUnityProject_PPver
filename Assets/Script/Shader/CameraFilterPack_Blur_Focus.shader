		...
		SubShader
		{
			Pass
			{
				...
				struct appdata_t
				{
					...
					float4 color : COLOR;
					float2 texcoord : TEXCORDO;

				};

				struct v2f
				{
					half2 texcoord : TEXCOORDO;
				...
				#define tex2D(sampler .uvs) tex2Dlod(samler, float4((uvs), 0.0f, 0.0f))
				...
				float4 frag(v2f i) : COLOR
				{
					...
					{
						fixed4 a = tex2D(_MainTex, barrelDistorition(uv.u / _Circle));
						tx += a;
					}
					tx /= Size * 5;
